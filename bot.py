...
async def deliveries_for_message_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        return await update.message.reply_text("⛔ অনুমতি নেই।")
    if not context.args:
        return await update.message.reply_text("ব্যবহার: /deliveries <message_row_id>")
    try:
    ...
    conn.close()
    if not rows:
        return await update.message.reply_text("এই আইডির জন্য কোনো ডেলিভারি পাওয়া যায়নি।")
...
def main():
    ...
    # command handlers (Bangla-friendly)
    app.add_handler(CommandHandler("start", start_cmd))
    app.add_handler(CommandHandler("help", help_cmd))
    app.add_handler(CommandHandler("সহায়তা", help_cmd))
    app.add_handler(CommandHandler("addadmin", addadmin_cmd))
    app.add_handler(CommandHandler("removeadmin", removeadmin_cmd))
    app.add_handler(CommandHandler("listadmins", listadmins_cmd))
    app.add_handler(CommandHandler("groups", groups_cmd))
    app.add_handler(CommandHandler("অবস্থা", status_cmd))
    ...
    # channel posts -> forward to groups only
    ...
    # .বার্তা regex for bangla short broadcast command
    app.add_handler(MessageHandler(filters.Regex(r'^\.বার্তা'), bangla_barta_handler))

    logger.info("Bot started — polling for updates...")
    app.run_polling(allowed_updates=["message", "edited_message", "channel_post", "my_chat_member", "chat_member"])

# ---------------- Keep-alive small web server (for hosting that expects HTTP) ----------------
